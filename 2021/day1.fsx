...
let fileMeasurements = File.ReadAllLines ("./data/day1.txt") |> Array.map int
let testMeasurements = [| 199; 200; 208; 210; 200; 207; 240; 269; 260; 264 |] 

let findCountOfIncreasingMeasurements measures =
    Array.windowed 2 measures 
    |> Array.filter (fun pair -> pair[0] < pair[1]) 
    |> Array.length

test <@ findCountOfIncreasingMeasurements testMeasurements = 7 @>
printfn $"Part 1: {findCountOfIncreasingMeasurements fileMeasurements}"
...
let findThreeMeasurementsCountIncreasing measures =
    Array.windowed 3 measures |> Array.map Array.sum |> findCountOfIncreasingMeasurements
    
test <@ findThreeMeasurementsCountIncreasing testMeasurements = 5 @>